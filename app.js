...
function openDetail(week, day) {
  ...
  $("toggleDone").addEventListener("click", () => {
    ...
    if (cur.done) {
      celebrate();
      toast(!isRaceDetail ? DONE[Math.floor(Math.random() * DONE.length)] : w.finish ? "🏁 17 km gelopen! Wat een opbouw, maatje!" : w.race ? "🏅 Finisher in Antwerpen! Wat een race, maatje!" : w.tuneup ? "🏁 Wedstrijd voltooid, sterk gepacet!" : DONE[Math.floor(Math.random() * DONE.length)]);
    }
    closeDetail();
  });
